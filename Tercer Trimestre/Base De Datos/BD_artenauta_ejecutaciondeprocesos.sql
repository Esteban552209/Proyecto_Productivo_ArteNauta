...
CREATE TABLE Roles (
    ...
    nombre_rol ENUM('Usuario', 'Artista', 'Administrador')
);
...
-- 1
SET @email_login = 'juan1@gmail.com';
SET @pass_intento = '1234'; 

SELECT clave INTO @hash_db FROM Usuarios WHERE email = @email_login;

-- 2
CALL sp_validar_password(@pass_intento, @hash_db, @resultado);

-- 3
SELECT IF(@resultado, 'ACCESO CONCEDIDO', 'CONTRASEÑA INCORRECTA') AS login_status;
