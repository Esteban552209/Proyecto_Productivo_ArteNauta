...
function PerfilUsuario({ usuario, setSeccion }) {
  ...
  const handleGuardar = async () => {
    try {
      const res = await fetch(`http://localhost:3002/usuarios/${usuario.id}`, {
        method: "PATCH",
      ...
      localStorage.setItem("usuario", JSON.stringify({ ...usuario, ...actualizado }));
      setEditando(false);
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl shadow p-6 mb-6 flex flex-col sm:flex-row gap-6 items-start">
        ...
        <div className="flex flex-col items-center gap-2 min-w-[90px]">
          ...
          <span className={`text-xs font-semibold px-3 py-1 rounded-full capitalize ${rolColor[rolLabel] || rolColor.usuario}`}>
            {rolLabel}
          </span>
        </div>
        ...
        <div className="flex-1 w-full">
          {editando ? (
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm mb-3">@{(usuario?.nombre || "usuario").toLowerCase()}</p>

              <div className="flex flex-col gap-1 text-sm text-gray-600 mb-4">
                {usuario?.correo   && <span><span className="font-medium">Correo:</span> {usuario.correo}</span>}
                {usuario?.telefono > 0 && <span><span className="font-medium">Teléfono:</span> {usuario.telefono}</span>}
              </div>
