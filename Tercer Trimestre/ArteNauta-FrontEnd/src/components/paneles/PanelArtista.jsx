...
function PanelArtista({ setVista }) {
    ...
    useEffect(() => {
        const cargarMisObras = async () => {
            ...
            try {
                ...
                const misPublicaciones = data.filter((obra) => obra.id_artista === usuario.id);
                setObras(misPublicaciones);
            } catch (error) {
    ...
    const handleEliminar = async (id) => {
        ...
        try {
            const res = await fetch(`http://localhost:3002/publicaciones/${id}`, { method: "DELETE" });
                        ...
                        <div className="mt-8 bg-white rounded-2xl shadow p-6">
                            ...
                            {obras.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                                    {obras.map((obra) => (
                                        <div key={obra.id} className="border border-gray-100 rounded-xl p-4 relative">
                                            <button
                                            ...
                                            <p className="font-semibold text-gray-700">{obra.Titulo}</p>
                                            <p className="text-sm text-gray-400 mt-1">{obra.Descripcion}</p>
                                        </div>
                                    ))}
                                </div>
                            )}
