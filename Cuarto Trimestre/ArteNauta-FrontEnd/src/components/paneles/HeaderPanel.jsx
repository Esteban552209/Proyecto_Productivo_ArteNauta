...
function HeaderPanel({
    ...
    const idRol = Number(usuario?.id_rol); // Integrado de tu rama local

    // Estados de la master (Rama-de-Bryam) para notificaciones
    const [notifCount, setNotifCount] = useState(0);
    const [notifPreview, setNotifPreview] = useState([]);
    const [dropdownOpen, setDropdownOpen] = useState(false);
    const dropdownRef = useRef(null);

    // Mapeo de roles de tu rama
    const rolLabel = {
    ...
    // Lógica de notificaciones de la master
    ...
    return (
        <div>
            <header className="bg-gradient-to-r from-cyan-900 to-cyan-400 text-white p-4 flex justify-between items-center shadow-lg">
                ...
                {/* IZQUIERDA: Logo + Botón Hamburguesa + Rol */}
                ...
                <nav className="flex gap-2 items-center">
                    ...
                    {/* Botones comunes de Usuario y Artista */}
                    {(idRol === 1 || idRol === 2) && (
                    ...
                    {/* Botón exclusivo de Artista */}
                    {idRol === 2 && (
                        <button onClick={onSubirArte} className="px-4 py-2 rounded font-medium transition bg-cyan-600 hover:bg-cyan-900 cursor-pointer">
                            Subir Arte
                        </button>
                    )}

                    {/* Botones exclusivos de Admin (de la master) */}
                    {idRol === 3 && (
                        <>
                    ...
                    {/* 🔔 Campanita y Notificaciones de la Master */}
            ...
            {/* Sidebar del Administrador de tu rama */}
            {idRol === 3 && (
                <AdminSidebar
                    seccion={seccion}
